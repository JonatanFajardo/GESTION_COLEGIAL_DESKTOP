...
namespace Gestion.Colegial.UI.Forms.Duraciones
{
    public partial class Add : Add_Base
    {
        ...
        {
            Answer ServiceInsert = await DuracionesServices.ListOne(id);
            if (!ServiceInsert.Access)
            {
                PR_tbDuraciones_FindResult Entity = ServiceInsert.Data;
                txtDescripcion.Texts = Entity.Dur_Descripcion;
            }
            else
        ...
        public void load()
        {
            ...
            string ModifiDia = "ModifiDia Duraciones";
            if (_id.Equals(0))
            {
