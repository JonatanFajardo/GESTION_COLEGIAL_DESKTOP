...
namespace Gestion.Colegial.UI.Forms.Dias
{
    public partial class List : FormBase
    {
        ...
        public void DataGridViewFill(string buscar = "")
        {
            ...
            // Peticion de la data
            DataTable data = DiasServices.List(buscar);
            //if (data==null)
            //{
        ...
        public override void CellContentDGV(DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridViewJN1.Rows[e.RowIndex].Cells[2].Selected)
            {
                ...
                if (Warning.isOk())
                {
                    tbDias objDias = new tbDias()
                    {
                        Dia_Id = (int)dataGridViewJN1.Rows[e.RowIndex].Cells[e.ColumnIndex + 1].Value,
                        //Dia_Descripcion = dataGridViewJN1.Rows[e.RowIndex].Cells[e.ColumnIndex + 2].Value.ToString()
                    };
                    Boolean resultService = DiasServices.Remove(objDias.Dia_Id);
                    DataGridViewFill();
                    if (resultService)
                    {
        ...
        //    }
        //    System.Collections.Generic.List<tbDias> lista = new System.Collections.Generic.List<tbDias>();
        //    lista.Add(new tbDias()
        //    {
        //        //mda_Descripcion = 
        //    });
