...
namespace Gestion.Colegial.UI.Forms.Dias
{
    public partial class Add : Add_Base
    {
        ...
        {
            Answer ServiceInsert = await DiasServices.ListOne(id);
            if (!ServiceInsert.Access)
            {
                PR_tbDias_FindResult Entity = ServiceInsert.Data;
                txtDescripcion.Texts = Entity.Dia_Descripcion;
            }
            else
        ...
        public void load()
        {
            ...
            string ModifiDia = "ModifiDia Dias";
            if (_id.Equals(0))
            {
