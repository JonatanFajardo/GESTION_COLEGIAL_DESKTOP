...
namespace Gestion.Colegial.UI.Forms.Secciones
{
    public partial class Add : Add_Base
    {
        ...
        {
            Answer ServiceInsert = await SeccionesServices.ListOne(id);
            if (!ServiceInsert.Access)
            {
                PR_tbSecciones_FindResult Entity = ServiceInsert.Data;
                txtDescripcion.Texts = Entity.Sec_Descripcion;
            }
            else
