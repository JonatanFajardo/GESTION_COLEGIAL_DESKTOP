...
namespace Gestion.Colegial.UI.Forms.Parciales
{
    public partial class Add : Add_Base
    {
        ...
        {
            Answer ServiceInsert = await ParcialesServices.ListOne(id);
            if (!ServiceInsert.Access)
            {
                PR_tbParciales_FindResult Entity = ServiceInsert.Data;
                txtDescripcion.Texts = Entity.Pac_Descripcion;
            }
            else
