...
namespace Gestion.Colegial.UI.Forms.Titulos
{
    public partial class List : FormBase
    {
        ...
        public void DataGridViewFill(string buscar = "")
        {
            ...
            // Peticion de la data
            DataTable data = TitulosServices.List(buscar);
            //if (data==null)
            //{
        ...
        public override void CellContentDGV(DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridViewJN1.Rows[e.RowIndex].Cells[0].Selected)
            {
                ...
                tbTitulos objTitulos = new tbTitulos()
                {
            ...
            if (dataGridViewJN1.Rows[e.RowIndex].Cells[2].Selected)
            {
                ...
                if (Warning.isOk())
                {
                    tbTitulos objTitulos = new tbTitulos()
                    {
                        Tit_Id = (int)dataGridViewJN1.Rows[e.RowIndex].Cells[e.ColumnIndex + 1].Value,
                        //Tit_Descripcion = dataGridViewJN1.Rows[e.RowIndex].Cells[e.ColumnIndex + 2].Value.ToString()
                    };
                    Boolean resultService = TitulosServices.Remove(objTitulos.Tit_Id);
                    DataGridViewFill();
                    if (resultService)
                    {
        ...
        //    }
        //    System.Collections.Generic.List<tbTitulos> lista = new System.Collections.Generic.List<tbTitulos>();
        //    lista.Add(new tbTitulos()
        //    {
        //        //mda_Descripcion = 
        //    });
