...
namespace Gestion.Colegial.Business.Services
{
    public class NotasServices
    {
        ...
        public static async Task<Answer> List()
        {
            ...
            {
                //Peticion.
                List<PR_tbNotas_ListResult> ApiResult = await ApiRequests.List<PR_tbNotas_ListResult>(ApiUrl.Notas.List);
        ...
        public static async Task<Answer> ListOne(int identifier)
        {
            try
            {
                PR_tbNotas_FindResult ApiResult = await ApiRequests.Find<PR_tbNotas_FindResult>(ApiUrl.Notas.Find, identifier);
                if (ApiResult is null)
                    goto ErrorResult;
        ...
        public static async Task<Boolean> Add(tbNotas entity)
        {
            try
            {
            return await ApiRequests.Create(ApiUrl.Notas.Create, entity);}
            catch (Exception error)
            {
        ...
        public static async Task<Boolean> Edit(tbNotas entity)
        {
            try
            {
            return await ApiRequests.Edit(ApiUrl.Notas.Update, entity);
        }
            catch (Exception error)
            {
               return ErrorLog.Incidents(error);
